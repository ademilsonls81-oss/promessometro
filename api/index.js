...
function db(type) {
  const url = process.env.VITE_S_URL || process.env.VITE_SUPABASE_URL || 'https://liqutcjzzrqstivvfele.supabase.co';
...
export default async function handler(req, res) {
  ...
  if (path === '/api/health') {
    try {
      return res.status(200).json({ status: 'ok', timestamp: new Date().toISOString(), env: !!process.env.VITE_SUPABASE_URL });
    } catch (e) {
      return res.status(500).json({ error: 'Health check failed: ' + e.message });
    }
  }
  ...
  if (path.startsWith('/api/politicians/ranking') && method === 'GET') {
    try {
      ...
      let query = db().from('politicians').select('id, name, role, state, party, slug, photo_url, is_active');
      if (!includeInactive) {
  ...
  if (path === '/api/stats' && method === 'GET') {
    const { count: politiciansCount } = await db().from('politicians').select('*', { count: 'exact', head: true });
    const { count: activePoliticiansCount } = await db().from('politicians').select('*', { count: 'exact', head: true }).eq('is_active', true);
    const { count: promisesCount } = await db().from('promises').select('*', { count: 'exact', head: true });
  ...
  if (path.startsWith('/api/politician/') && method === 'GET') {
    ...
    if (promises && promises.length > 0) {
      ...
      const [{ data: evaluations }, { data: evidences }] = await Promise.all([
        db().from('promise_explanations').select('*').in('promise_id', promiseIds).eq('is_latest', true),
        db().from('promise_evidences').select('*').in('promise_id', promiseIds)
      ]);
  ...
  if (path === '/api/promises/submit' && method === 'POST') {
    try {
      ...
      if (!existingPolitician) {
        const { error: politicianError } = await db().from('politicians').insert({
          nome: cleanName,
          cargo: finalPosition,
          estado: state || null,
          partido: null
      ...
      const { data: inserted, error } = await db().from('promises').insert({
  ...
  if (path === '/api/seed-evaluations' && method === 'POST') {
    const supabaseService = db("admin");
