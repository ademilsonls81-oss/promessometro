...
export function getUrlDomain(url: string | null | undefined): string | null {
  if (!url) return null;
  try {
...
export function classifySource(url: string | null | undefined): number {
...
export function sortByLevel<T extends { url?: string | null }>(evidencias: T[]): T[] {
...
export function prioritizeSources<T extends { url?: string | null; descricao?: string | null }>(
  evidencias: T[],
  maxSources = 3
): (T & { nivel: number; nivelLabel: string })[] {
  ...
  const deduped: (typeof withLevel)[number][] = [];
