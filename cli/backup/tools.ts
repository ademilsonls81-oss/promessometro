...
export const SLASH_COMMANDS = [
  ...
  { name: '/connect <provider> <apiKey>', description: 'Conecta um provider com a chave API do usuario' },
  { name: '/connect ollama', description: 'Ativa modo local via Ollama' },
...
export function buildSystemPrompt(cwd: string) {
  return `Voce e um coding agent do AI Feast Engine. PROJETO: ${cwd} | OS: ${process.platform}
FERRAMENTAS:
- read_file {path}
...
USO: <tool>NOME</tool><args>JSON</args>
Leia arquivos antes de editar.
Se o usuario pedir ajuda sobre a interface, mencione estes comandos:
